...
def build(in1s, in2s, hidden_dim, state_dim, environment_row, environment_col, environment_depth, argument_num, argument_depth, default_argument_num, program_embedding_size, program_size, batch_size=1):
    scratchPads = []
    for i in range(batch_size):
        scratchPads.append(ScratchPad(in1s[i], in2s[i], environment_row, environment_col))
    envs = [scratchPads[i].get_env(environment_row, environment_col, environment_depth) for i in range(batch_size)]
    task = AdditionCore(scratchPads, envs, hidden_dim, state_dim, environment_row, environment_col, environment_depth, argument_num, argument_depth, default_argument_num, program_embedding_size, program_size, batch_size=1)
    return task

class AdditionCore(TaskBase):
    def __init__(self, scratchPads, envs, hidden_dim, state_dim, environment_row, environment_col, environment_depth, argument_num, argument_depth, default_argument_num, program_embedding_size, program_size, batch_size=1):
        super(AdditionCore, self).__init__(envs, state_dim, batch_size)
        # config params
        ...
        self.scratchPads = copy.copy(scratchPads)
        # for f_enc
    ...
    def init_scratchPad(self, in1s, in2s):
        for i in range(self.batch_size):
            self.scratchPads[i].init_scratchpad(in1s[i], in2s[i])
    ...
    def f_enc(self, args):
        env = [self.scratchPads[i].get_env() for i in range(self.batch_size)]
    ...
    def f_env(self, prog_ids, args):
        for i in range(self.batch_size):
            ...
            if prog_id == MOVE_PID or prog_id == WRITE_PID:
                self.scratchPads[i].execute(prog_id, args)
        return [self.scratchPads[i].get_env(self.environment_row, self.environment_col, self.environment_depth)]