...
def build(in1s, 
    ...
    envs = [scratch_pads[i].get_env(environment_row, environment_col, environment_depth) for i in range(batch_size)]
...
class AdditionTask(TaskBase):
    ...
    def f_env(self, prog_ids, args):
        ...
        return [self.scratch_pads[i].get_env(self.environment_row, self.environment_col, self.environment_depth) for i in range(self.batch_size)]