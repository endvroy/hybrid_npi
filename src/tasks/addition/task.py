...
class AdditionTask(TaskBase):
    ...
    def f_enc(self, task_params, args):
        ...
        # print(merge)
        elu = F.elu(task_params.fenc1(merge))
        elu = F.elu(task_params.fenc2(elu))
        out = task_params.fenc3(elu)
        return out

    def get_program_embedding(self, task_params, prog_id):
        embedding = task_params.fenv1(prog_id)
        return embedding
