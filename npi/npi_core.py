...
class NPICore(nn.Module):
    def __init__(self,
        ...
        self.last_lstm_state = torch.zeros(1, 1, self.hidden_dim), \
    ...
    def forward(self, state, prog):
        ...
        ret = F.sigmoid(self.ret_fc(lstm_h).squeeze(0))
        pkey = F.tanh(self.pkey_fc(lstm_h).squeeze(0))
        args = F.tanh(self.args_fc(lstm_h).squeeze(0))
